...
const BattleStatus = ({ playerBattleStatus, playerBattleInfo, currentLocation }) => {
  ...
  const isSameLocation =
    Number(currentLocation.X) === Number(playerBattleInfo.coordX) &&
    Number(currentLocation.Y) === Number(playerBattleInfo.coordY)

  const [handleClick] = useModal(
    <BattleModal battle={battleId} status={playerBattleStatus} currentLocation={isSameLocation} />,
  )
  ...
  return (
    <Body>
      ...
      {playerBattleStatus !== '0' ? (
        <BattleButton type="button" onClick={handleClick}>
          BATTLE {battleId} <br /> {startTime}
        </BattleButton>
      ) : (
        ''
      )}
